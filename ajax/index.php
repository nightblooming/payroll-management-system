...
function EditEmployeeDetailsByID() {
    ...
    if ( !empty($first_name) && !empty($last_name) && !empty($dob) && !empty($gender) && !empty($merital_status) && !empty($nationality) && !empty($address) && !empty($city)  && !empty($country) && !empty($email) && !empty($mobile) && !empty($identity_doc) && !empty($identity_no) && !empty($emp_type) && !empty($joining_date)  && !empty($designation) && !empty($department)  && !empty($bank_name) && !empty($account_no)  ) {
    	$updateEmp = mysqli_query($db, "UPDATE `" . DB_PREFIX . "employees` SET `first_name` = '$first_name', `last_name` = '$last_name', `dob` = '$dob', `gender` = '$gender', `merital_status` = '$merital_status', `nationality` = '$nationality', `address` = '$address', `city` = '$city',  `country` = '$country', `email` = '$email', `mobile` = '$mobile', `identity_doc` = '$identity_doc', `identity_no` = '$identity_no', `emp_type` = '$emp_type', `joining_date` = '$joining_date',  `designation` = '$designation', `department` = '$department',  `bank_name` = '$bank_name', `account_no` = '$account_no' WHERE `emp_id` = $emp_id");
	    if ( $updateEmp ) {
...
function GeneratePaySlip() {
    ...
    if ( !empty($emp_code) && !empty($pay_month) ) {
	    ...
	    $html .= '<td colspan="3">: ' . ($empLeave['workingDays'] - $empLeave['withoutPay']) . '/' . $empLeave['workingDays'] . ' Days</td>';
	    $html .= '</tr>';
	    $html .= '</table>';
